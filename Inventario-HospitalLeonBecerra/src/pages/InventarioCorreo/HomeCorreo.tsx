...
class HomeCorreo extends React.Component<any, any> {
  ...
  asignar_parametros = (name: any, value: any) => {
    if (this.state.mounted) this.setState({ parametros: { ...this.state.parametros, [name]: value } });
  }

  clearReload() {
    if (this.state.mounted) {

      this.setState({ parametros: { page_size: 10, page_index: 0 }, popOver: false });
  ...
  componentDidMount = () => {
    this.setState({ showLoading: true, mounted: true })
    this.cargar_correos(true);
  }
    ...
    return (
      <IonPage>
        <IonHeader>
          <IonToolbar color="primary">
            ...
            <IonButtons slot="end">
              ...
              <IonButton onClick={() =>  { if (this.state.mounted) this.setState({ popOver: true })}}><IonIcon icon={options}></IonIcon></IonButton>
            </IonButtons>
            <IonPopover
              ...
              <IonList>
                <IonItem>
                  ...
                  <IonSelect value={this.state.parametros.departamento} okText="Ok" cancelText="Cancelar" name="departamento"
                    onIonChange={(e: any) =>  { if (this.state.mounted) this.asignar_parametros(e.target.name, e.target.value)}}>
                ...
                <IonItem>
                  ...
                  <IonDatetime value={this.state.parametros.fecha} doneText="Ok" cancelText="Cancelar" name="fecha" onIonChange={(e: any) =>   { if (this.state.mounted) this.asignar_parametros(e.target.name, e.target.value.substring(0, 10))}}
                    placeholder="Fecha" displayFormat="DD/MM/YYYY"
                  ></IonDatetime>
                </IonItem>
              </IonList>
              <div className="ion-text-center ion-margin">
                <IonButton expand="block" size="small" onClick={() =>  { if (this.state.mounted) this.handle_aplicar()}}>Aplicar</IonButton>
                <IonButton expand="block" size="small" onClick={() =>  { if (this.state.mounted) this.setState({ popOver: false })} }>Cancelar</IonButton>
                <IonButton expand="block" size="small" onClick={() =>  { if (this.state.mounted) this.clearReload()}} >Limpiar</IonButton>
              </div >
            </IonPopover>
          </IonToolbar>
        </IonHeader>
        <IonContent>
          ...
          <IonRefresher slot="fixed" onIonRefresh={(e: any) =>  { if (this.state.mounted) this.doRefresh(e, 0)}}>
            <IonRefresherContent refreshingSpinner="circles">
            </IonRefresherContent>
          </IonRefresher>

          <IonSearchbar placeholder={"Buscar por empleado"} onIonBlur={this.buscar_por_empleado} onIonChange={(e: any) => { if (this.state.mounted)  this.asignar_parametros("empleado", e.target.value)}}
          ...
          <IonInfiniteScroll disabled={this.state.disable_load} threshold="100px"
            onIonInfinite={(e: any) =>  { if (this.state.mounted) this.doRefresh(e, this.state.parametros.page_index + 1)}}
